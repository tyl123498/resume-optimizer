...
# Set LLM_PROVIDER=openai to use OpenAI-compatible API (DeepSeek, etc.)
# Default is "ollama" for local inference
...
# OpenAI-compatible settings (DeepSeek / 硅基流动 / etc.)
...
def get_llm():
    """Get or create the LLM instance based on provider configuration."""
    global _llm
    if _llm is not None:
        return _llm
...
async def analyze_resume(resume_text: str, jd_text: str) -> AnalysisResult:
    try:
        result_text = await analysis_chain.ainvoke({
...
async def generate_resume(resume_text: str, jd_text: str) -> GenerateResult:
    try:
        result_text = await generation_chain.ainvoke({
            "resume_text": resume_text,
            "jd_text": jd_text,
        })
        ...
        return GenerateResult(
            optimized_resume=f"# Generation Failed\n\nError: {str(e)}\n\nPlease try again."
        )
